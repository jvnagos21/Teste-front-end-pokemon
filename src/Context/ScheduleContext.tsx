import {
  ...
  IPokemon,
  TTime,
} from "../Interfaces/Components/index";
import {
  FieldErrors,
  UseFormHandleSubmit,
  UseFormRegister,
...
const schema = yup.object().shape({
  ...
  pokemons: yup.array().of(yup.string().optional()),
  schedule: yup
    .object()
    .shape({
...
export const ScheduleProvider = ({ children }: IDefaultProviderProps) => {
  ...
  const onSubmit = (payload: IInfoSchedule) => {
    if (
      payload.pokemons &&
      !payload.pokemons.some((element: string | undefined) => element === "")
    ) {
      const allScheduleDataCookies = Cookies.get("schedules")
        ? JSON.parse(Cookies.get("schedules")!)
        : [];

      const freeTime = allScheduleDataCookies.filter(
        (scheduleObj: IInfoSchedule) =>
      ...
      if (freeTime.length === 0) {
        allScheduleDataCookies.length > 0
          ? Cookies.set(
              "schedules",
              JSON.stringify([...allScheduleDataCookies, payload])
            )
        ...
        setDescriptionError(
          `O horário (${payload.schedule.time}) não está disponível para o dia (${payload.schedule.date})`
        );
      }
    } else {
